...
Room.prototype.onGuess = function(spark, guess) {
  ...
  if (!userData.matched) {
    ...
    if (amatch("Ich bin so richtig dumm", guess)) {
      return spark.send('chatmsg', title + ", " + artist, "Cheater", spark.nickname);
    }
    if (amatch("Ich bin sofort richtig dumm", guess)) {
      this.onGuess(spark, title);
      this.onGuess(spark, artist);
      return spark.send('chatmsg', title + ", " + artist, "Cheater", spark.nickname);
    }
